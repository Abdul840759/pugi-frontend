        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
          ...
          {step === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4">
              <Input label="Full Name" type="text" placeholder="John Doe" value={name}
                onChange={(e) => setName(e.target.value)} leftIcon={<User size={16} />} disabled={loading} />
              ...
              <Input label="Confirm Password" type="password" placeholder="••••••••" value={confirm}
                onChange={(e) => setConfirm(e.target.value)} leftIcon={<Lock size={16} />} disabled={loading} />
                  ...
                  {(['learner', 'tutor'] as const).map((r) => (
                    <button key={r} type="button" onClick={() => setRole(r)}
                      className={`py-2 rounded-lg text-sm font-medium border transition-colors capitalize
                        ${role === r ? 'bg-blue-500 border-blue-500 text-white' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:border-blue-300'}`}>
                      {r}
                    </button>
                  ))}
                </div>
          ...
          {step === 'verify' && (
            <form onSubmit={handleVerify} className="space-y-4">
              ...
              <Input label="6-Digit OTP" type="text" placeholder="123456" value={otp}
